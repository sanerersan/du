...
func (wg *WaitGroupWapper) Run(fn func(string),inData string) {
	wg.Add(1)
...
var capUnits = [...]string{"B","KB","MB","GB","TB"}
func GetHumanReadableString(cap int64) string {
	f := float64(cap)
	for i,unit := range capUnits {
		if (len(capUnits) == i) || (f < 1024) {
